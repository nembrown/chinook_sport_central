...
yearMonth2_season_north_aabm<- yearMonth2_season %>% 
  filter(str_detect(finescale_fishery, "NBC AABM"))%>%
  filter(season %in% c("spring", "fall"))
...
```{r Modelling NBC AABM Summer subsetting, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
...
```{r Modelling NBC AABM Summer, message=FALSE, warning=FALSE, results='hold'}
#### Adding confidence intervals based on model to a dataframe for plotting purposes
#based on this blog: https://fromthebottomoftheheap.net/2018/12/10/confidence-intervals-for-glms/
...
```{r forloop seasonal NBC AABM Summer, results='asis', echo=FALSE}
...
cat("\n\n")
 ...
 m<-ggplot(Summer_north_aabm_combined %>% filter(finescale_fishery==fishery_name_north_aabm[i])) +
    geom_point(size=2.5,  aes(y=historic, x=YEAR,col=status, fill=status, shape=pred_cat))+
    geom_line(aes(y=historic, x=YEAR,col=status, linetype = "Unfiltered creel and logbook"))+ 
    scale_shape_manual(values=c(1,1))+
    new_scale("shape")+
...
yearMonth2_season_north_isbm<- yearMonth2_season %>% 
  filter(str_detect(finescale_fishery, "NBC ISBM"))%>%
  filter(season %in% c("spring", "fall"))
...
```{r Modelling Summer NBC ISBM subsetting, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
...
```{r Modelling Summer NBC ISBM, message=FALSE, warning=FALSE, results='hold'}
...
```{r forloop summer NBC ISBM, results='asis', echo=FALSE}
...
cat("\n\n")
 ...
 m<-ggplot(Summer_north_isbm_combined %>% filter(finescale_fishery==fishery_name_north_isbm[i])) +
    geom_point(size=2.5,  aes(y=historic, x=YEAR,col=status, fill=status, shape=pred_cat))+
    geom_line(aes(y=historic, x=YEAR,col=status, linetype = "Unfiltered creel and logbook"))+ 
    scale_shape_manual(values=c(1,1))+
    new_scale("shape")+
