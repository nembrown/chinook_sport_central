...
-   JDF summer best predicted by summer.
-   JDF spring best predicted by month 4, not summer
-   JDF fall best predicted by summer and not month 9
...
CA_JDF_Spring_Summer<-ggplot(Sport_mark_rate_finescale_combined %>% filter(YEAR %in% c(2013:2023), finescale_fishery=="CA JDF S SPRING"), aes(x=creel_plus_summer, y= catch_estimate, col=status, fill=status))+geom_point()+geom_abline(slope=1)+
  geom_smooth(method="lm") + facet_wrap(~status, scales="free") + ggtitle("CA JDF S") + theme_bw() + scale_colour_viridis_d() + scale_fill_viridis_d()
