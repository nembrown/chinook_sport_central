...
```{r Pulling from CREST and NBC csv}
...
Season_model_gamma_drop_season<- glm(formula = catch_estimate+3 ~creel_plus_summer*mark_status*kept_status*finescale_fishery_old*creel_effort,  family=Gamma(link = "log"), data = Season_south)
...
-   Further investigation showed that the best model dropped kept status only, no further terms.
...
#####kept and effort plus other terms:
# 
...
-   Next, we investigated if the fully interactive model is better than a subset of interactions. Since we had too many interactive terms for the dredge function to iterate through, we first investigated dropping both kept and effort (since the model dropping effort was close in AIC), and then added effort back in to get the final model specification.
...
-   A subset of these interactions was found to be the better fit. We then took this subset and added effort back in to see if effort fit in with a subset of terms and the model with effort was a better fit for the data.

-   Therefore the best model included summer creel, mark status, finescale fishery, creel effort, and season and various interactions of these terms

    $$
    catch = creel.summer+finescale.fishery+season+mark.status+                                               creel.summer:finescale.fishery + creel.summer:season + creel.summer:mark.status+                                               finescale.fishery:season + finescale.fishery:mark.status + season:mark.status +                                               creel.summer:finescale.fishery:mark.status + creel.summer:finescale.fishery:season 
    $$
...
#####
g<-ggplot(Season_south_combined %>% filter(finescale_fishery==fishery_name_south[i]), aes(x=creel_plus_summer, y= catch_estimate_predicted+1, col=mark_status, fill=mark_status))+geom_point(aes(shape=pred_cat, size=creel_effort))+
geom_smooth(method="glm", method.args = list(family= Gamma(link = "log")), fullrange=TRUE) + facet_wrap(~mark_status, scales="free") + ggtitle(paste(fishery_name_south[i])) + theme_bw() + scale_colour_viridis_d(option = "turbo")+  scale_fill_viridis_d(option = "turbo") +scale_size_continuous(range=c(1,3))
...
```{r forloop old finescale fisheries, results='asis', echo=FALSE}
...
#####
g<-ggplot(Season_north_aabm_combined %>% filter(finescale_fishery==fishery_name_north_aabm[i]), aes(x=historic_summer, y= catch_estimate_predicted+1, col=status, fill=status))+geom_point(aes(shape=pred_cat, size=historic_effort))+
geom_smooth(method="glm", method.args = list(family= Gamma(link = "log")), fullrange=TRUE) + facet_wrap(~status, scales="free") + ggtitle(paste(fishery_name_north_aabm[i])) + theme_bw() + scale_colour_viridis_d() + scale_fill_viridis_d() + scale_size_continuous(range = c(1, 3)) 
