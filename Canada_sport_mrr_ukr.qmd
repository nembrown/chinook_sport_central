...
### Juan de Fuca Strait Summer - CA JDF S SUMMER

-   Before 2013: good coverage creel 6-8, iREC fills in areas not creeled
-   Marked releases generally at lower end of prediction interval

```{r plot JDF Summer, echo=FALSE, message=FALSE, warning=FALSE}
library(patchwork)

CA_JDF_1<-ggplot()+
  geom_tile(data=yearMonth2 %>% filter(finescale_fishery=="CA JDF S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum),colour = "white") +
  scale_y_discrete(limits=rev)+
  geom_tile(data=yearMonth2  %>% filter(finescale_fishery=="CA JDF S SUMMER", MONTH %in% c(6:8)), aes(x=YEAR, y=as.factor(MONTH), fill = sum_historic),colour = "white") +
  guides(fill=guide_legend(title="creel and logbook estimates")) +
  scale_fill_gradient(low = col1, high = col2, na.value = colclear)+
  labs(title = "Coverage for CA JDF S SUMMER",
       x = "Year", y = "Month") +
  theme_bw() + theme_minimal() + geom_vline(xintercept = 2013)
...
CA_JDF_2<-ggplot()+
  ...
  scale_y_discrete(limits=rev)+
  guides(fill=guide_legend(title="creel and logbook estimates")) +
  geom_tile(data=yearMonth2%>% filter(finescale_fishery=="CA JDF S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_historic),colour = "white") +
  scale_fill_gradient(low = col1, high = col2, na.value = colclear) +
  new_scale_fill() +
  geom_tile(data=yearMonth_catch_estimate_2%>% filter(finescale_fishery=="CA JDF SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col3, high = col4, na.value = colclear)+
  guides(fill=guide_legend(title="creel plus irec estimates")) +
  new_scale_fill() +
  geom_tile(data=yearMonth_irec_2%>% filter(finescale_fishery=="CA JDF S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col5, high = col6, na.value = colclear)+
  guides(fill=guide_legend(title="irec estimates")) +
  labs(
       x = "Year", y = "Month") +
...
ggplot(Summer_south_combined %>% filter(finescale_fishery=="CA JDF S SUMMER"), aes(x=creel_plus_summer, y= catch_estimate_predicted, col=status, fill=status, shape=pred_cat))+geom_point(size=2.5)+geom_abline(slope=1)+
  geom_smooth(method="glm", method.args = list(family= Gamma(link = "log"))) + facet_wrap(~status, scales="free") + ggtitle("CA JDF S SUMMER") + theme_bw() + scale_colour_viridis_d() + scale_fill_viridis_d()


```

### Johnstone Strait - JNST S Summer 

-   Before 2013: good coverage creel 6-8, irec fills in Areas not creeled

-   Marked kept fish has a few points on the higher end of prediction interval

-   All categories have some predicted points at low end of interval

```{r, echo=FALSE, message=FALSE, warning=FALSE}

CA_JNST_1<-ggplot()+
  geom_tile(data=yearMonth2 %>% filter(finescale_fishery=="JNST S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum),colour = "white") +
  scale_y_discrete(limits=rev)+
  geom_tile(data=yearMonth2  %>% filter(finescale_fishery=="JNST S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_historic),colour = "white") +
  guides(fill=guide_legend(title="creel and logbook estimates")) +
  scale_fill_gradient(low = col1, high = col2, na.value = colclear)+
  labs(title = "Coverage for JNST S SUMMER",
       x = "Year", y = "Month") +
  theme_bw() + theme_minimal() + geom_vline(xintercept = 2013)
...
CA_JNST_2<-ggplot()+
  ...
  scale_fill_gradient(low = col1, high = col2, na.value = colclear) +
  new_scale_fill() +
  geom_tile(data=yearMonth_catch_estimate_2%>% filter(finescale_fishery=="JNST S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col3, high = col4, na.value = colclear)+
  guides(fill=guide_legend(title="creel plus irec estimates")) +
  new_scale_fill() +
  geom_tile(data=yearMonth_irec_2%>% filter(finescale_fishery=="JNST S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col5, high = col6, na.value = colclear)+
  guides(fill=guide_legend(title="irec estimates")) +
  labs(
       x = "Year", y = "Month") +
...
#Predicted model plot
ggplot(Summer_south_combined %>% filter(finescale_fishery=="JNST S SUMMER"), aes(x=creel_plus_summer, y= catch_estimate_predicted, col=status, fill=status, shape=pred_cat))+geom_point(size=2.5)+geom_abline(slope=1)+
  geom_smooth(method="glm", method.args = list(family= Gamma(link = "log"))) + facet_wrap(~status, scales="free") + ggtitle("JNST S SUMMER") + theme_bw() + scale_colour_viridis_d() + scale_fill_viridis_d()


```

### North Georgia Strait - NGS S SUMMER

-   Good coverage 6:8,

-   All categories of catch both much lower than prediction interval -- catch in NGS has increased dramatically. ds

```{r, echo=FALSE, message=FALSE, warning=FALSE}
CA_NGS_1<-ggplot()+
  geom_tile(data=yearMonth2 %>% filter(finescale_fishery=="NGS S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum),colour = "white") +
  scale_y_discrete(limits=rev)+
  geom_tile(data=yearMonth2  %>% filter(finescale_fishery=="NGS S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_historic),colour = "white") +
  guides(fill=guide_legend(title="creel and logbook estimates")) +
  scale_fill_gradient(low = col1, high = col2, na.value = colclear)+
  labs(title = "Coverage for NGS S SUMMER",
       x = "Year", y = "Month") +
  theme_bw() + theme_minimal() + geom_vline(xintercept = 2013)
...
CA_NGS_2<-ggplot()+
  ...
  scale_fill_gradient(low = col1, high = col2, na.value = colclear) +
  new_scale_fill() +
  geom_tile(data=yearMonth_catch_estimate_2%>% filter(finescale_fishery=="NGS S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col3, high = col4, na.value = colclear)+
  guides(fill=guide_legend(title="creel plus irec estimates")) +
  new_scale_fill() +
  geom_tile(data=yearMonth_irec_2%>% filter(finescale_fishery=="NGS S SUMMER"), aes(x=YEAR, y=as.factor(MONTH), fill = sum_catch_estimate),colour = "white") +
  scale_fill_gradient(low = col5, high = col6, na.value = colclear)+
  guides(fill=guide_legend(title="irec estimates")) +
  labs(
       x = "Year", y = "Month") +
...
#Predicted model plot
ggplot(Summer_south_combined %>% filter(finescale_fishery=="NGS S SUMMER"), aes(x=creel_plus_summer, y= catch_estimate_predicted, col=status, fill=status, shape=pred_cat))+geom_point(size=2.5)+geom_abline(slope=1)+
  geom_smooth(method="glm", method.args = list(family= Gamma(link = "log"))) + facet_wrap(~status, scales="free") + ggtitle("NGS S SUMMER") + theme_bw() + scale_colour_viridis_d() + scale_fill_viridis_d()
