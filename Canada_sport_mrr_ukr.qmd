...
```{r Modelling Summer BC fisheries, message=FALSE, warning=FALSE}
...
-   Forloop script for printing out all summer plots

```{r forloop summer, results='asis'}
...
## Southern BC Fall fisheries

-   The best model was the model without kept status, but with the the "fall" month (month 9 or 8 if 9 wasn't available)
...
#Adding predicted data
Summer_terminal_old<- Sport_mark_rate_finescale_combined %>% filter(YEAR %in% c(2005:2012), finescale_fishery %in% c("TCA JDF S SUMMER", "TJOHN ST S SUMMER", "TNGS S SUMMER", "TSGS S SUMMER", "TWCVI S SUMMER")) %>% ungroup() %>% mutate(pred_cat = "predicted") 
...
-   Forloop summer terminal

```{r forloop Summer terminal, results='asis' }
...
-   forloop fall terminal

```{r forloop Fall terminal, results='asis' }
