...
Commercial_mr2<- read.csv(here::here("commercial_mark_rate.csv")) %>% as_tibble()  %>% filter(AREA %in% c("Area 1", "Area 101", "Area 142", "Area 2", "Area 102")) %>% group_by(YEAR, AREA, MONTH) %>% summarise_if(is.numeric, sum, na.rm = TRUE) %>% mutate(marked_prop_commercial=ADCLIP_OBS/ADCLIP_SAMPLE) 
