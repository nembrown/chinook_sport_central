...
-   Pulled recreational catch data from CREST database: data sources include creel, iREC, lodge and log-book data.
...
-   Filtered based on high quality creel which meet the following criteria in a given PFMA and month:
...
-   Calculated mark rate for each data source based on number of marked and unmarked fish encountered.

-   Combined data sources using the following rule:
...
-   Blue borders indicate the estimate includes data from iREC, without the border is creel-only information

-   Green means go --\> green colours indicate higher mark rate
...
MR_post2019<-ggplot(data=PFMAMonth_estimate %>% filter(marked_prop_source!="no source"), aes(x=month(MONTH, label=TRUE), y=as.factor(AREA), fill = mr, col=marked_prop_source))+
...
MR_pre2019<-ggplot(data=PFMAMonth_estimate_past %>% filter( marked_prop_source!="no source"), aes(x=month(MONTH, label=TRUE), y=as.factor(AREA), fill = mr, col=marked_prop_source))+
  geom_tile(linewidth=1.5) +
  scale_colour_manual(values = c("lightblue", "#1C00ff00", "#1C00ff00"))+
  geom_text(aes(label=round(mr*100, 0)), col="black")+
  scale_fill_paletteer_c("ggthemes::Red-Green-Gold Diverging", direction=1)+
...
PFMAMonth_irec <- plyr::ddply(Sport_mark_rate_only %>% filter(YEAR %in% 2019:2023), c( "MONTH", "AREA"), summarise, mr = mean(marked_prop_irec_calibrated, na.rm=TRUE)) %>% drop_na()

PFMAMonth_irec_plot<-ggplot(data=PFMAMonth_irec, aes(x=month(MONTH, label=TRUE), y=as.factor(AREA), fill = mr))+
...
PFMAMonth_creel <- plyr::ddply(Sport_mark_rate_only %>% filter(YEAR %in% 2019:2023,  AREA %notin% c("Section PR-1", "Section PR-3")), c( "MONTH", "AREA"), summarise,
                         mr = mean(marked_prop_creel, na.rm=TRUE))%>% drop_na()

PFMAMonth_creel_plot<-ggplot(data=PFMAMonth_creel, aes(x=month(MONTH, label=TRUE), y=as.factor(AREA), fill = mr))+
  geom_tile(colour = "white") +
  geom_text(aes(label=round(mr*100, 0)))+
  scale_fill_paletteer_c("ggthemes::Red-Green-Gold Diverging", direction=-1)+
   scale_y_discrete(limits=rev)+
...
PFMAMonth_lodge_log <- plyr::ddply(Sport_mark_rate_only %>% filter(YEAR %in% 2019:2023), c( "MONTH", "AREA"), summarise,
                         mr = mean(marked_prop_lodge_log, na.rm=TRUE))%>% drop_na()

PFMAMonth_lodge_log_plot<-ggplot(data=PFMAMonth_lodge_log, aes(x=month(MONTH, label=TRUE), y=as.factor(AREA), fill = mr))+
  geom_tile(colour = "white") +
  geom_text(aes(label=round(mr*100, 0)))+
  scale_fill_paletteer_c("ggthemes::Red-Green-Gold Diverging", direction=-1)+
   scale_y_discrete(limits=rev)+
