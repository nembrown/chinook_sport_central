...
North_isbm_model_full<- glm(formula = catch_estimate + 1 ~ historic_summer*status*season,  family=gaussian, data = Season_north_isbm_no_nas)
summary(North_isbm_model_full)
res <- simulateResiduals(North_isbm_model_full, plot = T, quantreg=T)

#poisson
North_isbm_model_full_poisson<- glm(formula = catch_estimate + 1 ~historic_summer*status*season,  family=poisson, data = Season_north_isbm_no_nas)
res_pois <- simulateResiduals(North_isbm_model_full_poisson, plot = T, quantreg=T)
summary(North_isbm_model_full_poisson)

#gamma
North_isbm_model_full_gamma<- glm(formula = (catch_estimate+1) ~historic_summer*status*season,  family=Gamma(link = "log"), data=Season_north_isbm_no_nas, na.action = na.fail)
res_gam <- simulateResiduals(North_isbm_model_full_gamma, plot = T, quantreg=T)
summary(North_isbm_model_full_gamma)

AICtab(North_isbm_model_full,North_isbm_model_full_poisson, North_isbm_model_full_gamma)

#gamma is the best

#Now changing around model specification:
North_isbm_model_full_gamma<- glm(formula = catch_estimate+1 ~historic_summer*season*status,  family=Gamma(link = "log"), data = Season_north_isbm_no_nas, na.action = na.fail)
dd3<-dredge(North_isbm_model_full_gamma, fixed= ~ historic_summer)
subset(dd3, delta < 2)
plot(dd3, labAsExpr = TRUE)
#
summary(get.models(dd3, 1)[[1]])
...
summary(North_isbm_model_full_gamma_spec)

AICtab(North_isbm_model_full_gamma, North_isbm_model_full_gamma_spec)



### Selected model:

testDispersion(North_isbm_model_full_gamma_spec)
simulationOutput <- simulateResiduals(fittedModel = North_isbm_model_full_gamma_spec, plot = F)
residuals(simulationOutput)
residuals(simulationOutput, quantileFunction = qnorm, outlierValues = c(-7,7))
plot(simulationOutput)
plotResiduals(simulationOutput, Season_north_isbm_no_nas$historic_summer)

plotResiduals(simulationOutput, form = na.omit(Season_north_isbm$finescale_fishery_old))

testCategorical(simulationOutput, catPred = na.omit(Season_north_isbm$season))
